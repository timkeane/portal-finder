...
var GEOCLIENT_URL = 'https://maps.nyc.gov/geoclient/v1/search.json?app_key=' + APP_KEY + '&app_id=' + APP_ID;
...
function mapClicked(event){
	map.forEachFeatureAtPixel(event.pixel, function(feature, layer){
		var found = feature.get('BoroCD'); /* replace id with unigue id from data in layer.json */
	...
	source = new nyc.ol.source.Decorating(
		...
		[{
			extendFeature: function(){
				this.setId(this.get('BoroCD')); /* set the feature id using the unique id from layer.json */
			},
